...
if sha256 != '1a41c95be7427ddd3397249fde5be56dfd6f4a8cef20ab27a7a648f31e824dfb'
  puts "The sha256 of the loaded file does not match the expected value! You are probably trying to load a different SNR file. This may or may not work. Remove this check from the script to continue"
  exit
end
...
class OutFile
  ...
  def register(num)
    if num > 0xFF
      puts "Register #{num} (0x#{num.to_s(16)}) > 0xff"
    end
    "%rx#{num.to_s(16)}"
  end
  ...
  def ins_0x87(argument)
    nyi
    self << "instruction 0x87 (dialogue pipe wait?), argument: #{hex(argument)}"
  end

  def ins_0x89(argument, val1)
    nyi
    self << "instruction 0x89 (hide dialogue window?), argument: #{hex(argument)}, val1: #{val1}"
  end

  def ins_0x8a(argument)
    nyi
    self << "instruction 0x8a, argument: #{hex(argument)}"
  end

  def ins_0x8b(data)
    nyi
    self << "instruction 0x8b, argument: #{hex(data)}"
  end

  def ins_0x8d(data)
    nyi
    self << "instruction 0x8d, argument: #{hex(data)}"
  end

  def ins_0x8e_0x2(var1, var2)
    nyi
    self << "instruction 0x8d, var1: #{hex(var1)}, var2: #{hex(var2)}"
  end

  def ins_0x8f
    nyi
    self << "instruction 0x8f"
  end
  ...
  def register_signed_assign(reg, value)
    nyi
    self << "mov #{register(reg)}, #{value}"
  end

  def register_unsigned_assign(reg, value)
    nyi
    self << "mov #{register(reg)}, #{value} ;unsigned"
  end

  def register_add(reg, value)
    nyi
    self << "mov #{register(reg)}, #{register(reg)} + #{value}"
  end
  ...
  def register_or(reg, value)
    nyi
    self << "mov #{register(reg)}, #{register(reg)} [OR] #{value}"
  end

  def register_and(reg, value)
    nyi
    self << "mov #{register(reg)}, #{register(reg)} [AND] #{value}"
  end

  def register_0x08(reg, value) # perhaps mod?
    nyi
    self << "mov #{register(reg)}, #{register(reg)} [0x08] #{value}"
  end

  def register_add2(reg, value1, value2)
    nyi
    self << "mov #{register(reg)}, #{value1} + #{value2}"
  end
  ...
  def register_0x84(reg, value)
    nyi
    self << "mov #{register(reg)}, #{register(reg)} [0x84] #{value}"
  end

  def register_0x87(reg, value1, value2)
    nyi
    self << "mov #{register(reg)}, #{value1} [0x87] #{value2}"
  end
  ...
  def ins_0x40(val1, val2, val3)
    nyi
    self << "instruction 0x40, val1: #{val1}, val2: #{val2}, val3: #{val3}"
  end

  def calc(target, operations)
    nyi
    self << "calc #{target} := #{hex(operations)}"
  end

  def ins_0x43(val1, data)
    nyi
    self << "instruction 0x43 (store multiple?), val1: #{val1}, data: #{hex(data)}"
  end

  def ins_0x44(val1, val2, val3, data)
    nyi
    self << "instruction 0x44 (another kind of store string?), val1: #{val1}, val2: #{val2}, val3: #{val3}, data: #{hex(data)}"
  end

  def ins_0x45(val1, val2, data)
    nyi
    self << "instruction 0x45 (store multiple?), val1: #{val1}, val2: #{val2}, data: #{hex(data)}"
  end
  ...
  def call(addr, data)
    nyi
    self << "call #{address(addr)}, #{data.map(&:to_s).join(', ')}"
  end
  ...
  def conditional_jump(val1, val2, addr, comparison)
    self << "goto *#{address(addr)} if val1 #{comparison} val2"
  end
  ...
  def ins_0x48(addr)
    nyi
    self << "instruction 0x48 (gosub?), addr: #{address(addr)}"
  end

  def ins_0x49
    nyi
    self << "instruction 0x49 (return?)"
  end

  def ins_0x4a(value, targets)
    nyi
    self << "instruction 0x4a, value: #{value}, targets: #{addresses(targets)}"
  end

  def ins_0x4b(register, targets)
    nyi
    self << "instruction 0x4b, register: #{hex(register)}, targets: #{addresses(targets)}"
  end

  def ins_0x4c(data)
    nyi
    self << "instruction 0x4c, data: #{hex(data)}"
  end
  ...
  def ins_0x51(val1, val2, val3, val4, data)
    nyi
    self << "instruction 0x51, values: #{val1} #{val2} #{val3} #{val4}, data: #{data.map { |e| e.to_s(16) }}"
  end

  def ins_0x52
    nyi
    self << "instruction 0x52 (some kind of return?)"
  end
  ...
  def syscall(target)
    nyi
    self << "syscall #{hex(target)}"
  end

  def ins_0x82(data)
    nyi
    self << "instruction 0x82: #{hex(data)}"
  end

  def ins_0x83(mode, val)
    nyi
    self << "instruction 0x83, mode: #{hex(mode)}, val: #{val}"
  end

  def ins_0x85(val1)
    nyi
    self << "instruction 0x85, val1: #{val1}"
  end
  ...
  def ins_0x4d(val1, val2)
    nyi
    self << "instruction 0x4d (push?), val1: #{hex(val1)}, val2: #{val2}"
  end

  def ins_0x4e_0x01(val1)
    nyi
    self << "instruction 0x4e 0x01 (pop one value?), val1: #{hex(val1)}"
  end

  def ins_0x4e_0x02(val1, val2)
    nyi
    self << "instruction 0x4e 0x02 (pop two values?), val1: #{hex(val1)}, val2: #{hex(val2)}"
  end

  def ins_0xff(data)
    nyi
    self << "instruction 0xff (data section?) at 0x#{@offset.to_s(16)}, data: #{data}"
  end
  ...
  def resource_command_0x0(slot, val1, val2)
    nyi
    self << "resource command (0xc1) 0x0 (remove slot?), slot #{slot}, values: #{val1} #{val2}"
  end

  def resource_command_0x1(slot, val1, val2, val3, val4, val5, val6, val7)
    nyi
    self << "resource command (0xc1) 0x1 (load simple?), slot #{slot}, values: #{val1} #{val2} #{val3} #{val4} #{val5} #{val6} #{val7}"
  end

  def resource_command_0x2(slot, val1, val2, picture_index)
    nyi
    self << "resource command (0xc1) 0x2 (pic load?), slot #{slot}, values: #{val1} #{val2}, picture index: #{picture_index}"
  end

  def resource_command_0x3(slot, val1, val2, val3)
    nyi
    self << "resource command (0xc1) 0x3 (sprite_load?), slot #{slot}, values: #{val1} #{val2} #{val3}"
  end

  def resource_command_0x4(slot, val1, val2)
    nyi
    self << "resource command (0xc1) 0x4 (anime_load?), slot #{slot}, values: #{val1} #{val2}"
  end

  def resource_command_0x6_0x2(slot, val1)
    nyi
    self << "resource command (0xc1) 0x6 0x2, slot #{slot}, values: #{val1}"
  end

  def resource_command_0x6_0x3(slot, val1, val3)
    nyi
    self << "resource command (0xc1) 0x6 0x3, slot #{slot}, values: #{val1} #{val3}"
  end

  def resource_command_0x9(slot, val1, val2, val3)
    nyi
    self << "resource command (0xc1) 0x9 (special?), slot #{slot}, values: #{val1} #{val2} #{val3}"
  end

  def sprite_command_0x00(slot)
    nyi
    self << "sprite command (0xc2) 0x00 (z order?)"
  end

  def sprite_command_0x01(slot)
    nyi
    self << "sprite command (0xc2) 0x01 (alpha?)"
  end

  def sprite_command_0x12(slot)
    nyi
    self << "sprite command (0xc2) 0x12 (y resize?)"
  end

  def sprite_wait_0x00(val1, val2)
    nyi
    self << "sprite wait (0xc3) 0x00, values: #{val1} #{val2}"
  end

  def sprite_wait_0x01(val1, val2, val3)
    nyi
    self << "sprite wait (0xc3) 0x01 (alpha?), values: #{val1} #{val2} #{val3}"
  end

  def sprite_wait_0x02(val1, val2, val3)
    nyi
    self << "sprite wait (0xc3) 0x02, values: #{val1} #{val2} #{val3}"
  end

  def sprite_wait_0x03(val1, val2, val3, val4)
    nyi
    self << "sprite wait (0xc3) 0x03, values: #{val1} #{val2} #{val3} #{val4}"
  end

  def sprite_wait_0x04(val1, val2, val3)
    nyi
    self << "sprite wait (0xc3) 0x04, values: #{val1} #{val2} #{val3}"
  end

  def sprite_wait_0x05(val1, val2, val3, val4)
    nyi
    self << "sprite wait (0xc3) 0x05 (x pos?), values: #{val1} #{val2} #{val3} #{val4}"
  end

  def sprite_wait_0x06(val1, val2, val3, val4)
    nyi
    self << "sprite wait (0xc3) 0x06 (y pos?), values: #{val1} #{val2} #{val3} #{val4}"
  end

  def sprite_wait_0x07(val1, val2, val3, val4, val5)
    nyi
    self << "sprite wait (0xc3) 0x07, values: #{val1} #{val2} #{val3} #{val4} #{val5}"
  end

  def sprite_wait_0x0f(val1, val2, val3, val4, val5, val6)
    nyi
    self << "sprite wait (0xc3) 0x0f, values: #{val1} #{val2} #{val3} #{val4} #{val5} #{val6}"
  end

  def ins_0xc0(slot)
    nyi
    self << "instruction 0xc0, slot: #{hex(slot)}"
  end

  def ins_0xc4(target, data)
    nyi
    self << "instruction 0xc0, target: #{target}, data: #{data}"
  end

  def ins_0xc6(id1, id2)
    nyi
    self << "instruction 0xc6 (load sprite?), id1: #{id1}, id2: #{id2}"
  end

  def ins_0xc7(slot, command)
    nyi
    self << "instruction 0xc7 (some sprite command?), slot: #{slot}, command: #{hex(command)}"
  end

  def ins_0xc9
    nyi
    self << "instruction 0xc9 (some kind of marker?)"
  end

  def ins_0xca(register)
    nyi
    self << "instruction 0xca, slot/register: #{hex(register)}"
  end

  def ins_0xcb
    nyi
    self << "instruction 0xcb (waiting for something?)"
  end

  def ins_0xcc(val1)
    nyi
    self << "instruction 0xcc, val1: #{hex(val1)}"
  end

  def ins_0xcd(val1, val2)
    nyi
    self << "instruction 0xcd, val1: #{hex(val1)}, val2: #{hex(val2)}"
  end

  def ins_0xce(val1)
    nyi
    self << "instruction 0xce, val1: #{hex(val1)}"
  end

  def ins_0xd0; nyi; self << "instruction 0xd0 (special?)"; end
  def ins_0xd1; nyi; self << "instruction 0xd1 (special?)"; end
  def ins_0xd2; nyi; self << "instruction 0xd2 (special?)"; end
  def ins_0xd3; nyi; self << "instruction 0xd3 (special?)"; end
  def ins_0xd4; nyi; self << "instruction 0xd4 (special?)"; end

  def ins_0xe0(data)
    nyi
    self << "instruction 0xe0, data: #{data}"
  end
  ...
  def ins_0x90(data)
    nyi
    self << "instruction 0x90, data: #{hex(data)}"
  end

  def ins_0x91(val1)
    nyi
    self << "instruction 0x91, val1: #{hex(val1)}"
  end

  def ins_0x92(val1)
    nyi
    self << "instruction 0x92, val1: #{hex(val1)}"
  end

  def ins_0x95(channel, sfxid, flag, val1, val2, val3)
    nyi
    self << "instruction 0x95 (sfx related?), channel: #{hex(channel)}, sfxid: #{hex(sfxid)}, flag: #{hex(flag)}, val1: #{val1}, val2: #{val2}, val3: #{val3}"
  end

  def ins_0x96(channel, val1)
    nyi
    self << "instruction 0x96 (sfx fade?), channel: #{hex(channel)}, val1: #{val1}"
  end

  def ins_0x97(channel)
    nyi
    self << "instruction 0x97 (bgm related?), channel: #{hex(channel)}"
  end

  def ins_0x98
    nyi
    self << "instruction 0x98"
  end

  def ins_0x9a(val1, val2)
    nyi
    self << "instruction 0x9a (sound related?), val1: #{val1}, val2: #{val2}"
  end

  def ins_0x9b(val1, val2, val3, val4, val5)
    nyi
    self << "instruction 0x9b (rumble?), val1: #{val1}, val2: #{val2}, val3: #{val3}, val4: #{val4}, val5: #{val5}"
  end
  ...
  def section_title(type, str)
    nyi
    self << "section title: '#{str}', type: #{hex(type)}"
  end

  def ins_0xa1
    nyi
    self << "instruction 0xa1 (set timer?)"
  end

  def ins_0xa2(argument)
    nyi
    self << "instruction 0xa1 (clear timer & disable skip?), argument: #{hex(argument)}"
  end

  def ins_0xa3
    nyi
    self << "instruction 0xa3 (unset timer?)"
  end

  def ins_0xb0(val)
    nyi
    self << "instruction 0xb0 (marker?), val: #{hex(val)}"
  end

  def ins_0xb1(val1, data)
    nyi
    self << "instruction 0xb1, val1: #{hex(val1)}, data: #{hex(data)}"
  end

  def ins_0xb2(val)
    nyi
    self << "instruction 0xb2, val: #{hex(val)}"
  end

  def ins_0xbb(val)
    nyi
    self << "instruction 0xbb, val: #{hex(val)}"
  end

  def ins_0xb3; nyi; self << "instruction 0xb3"; end
  def ins_0xbd; nyi; self << "instruction 0xbd"; end
  def ins_0xbe; nyi; self << "instruction 0xbe"; end
  def ins_0xbf; nyi; self << "instruction 0xbf"; end
end
